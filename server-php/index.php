...
/**
 * Appointments API — front controller.
 *
 * Deployed to <document root>/api, so it is same-origin with the React app on
 * both appointments.aicountly.com and appointments.gh.aicountly.com.
 *
 * Routes:
 *   GET  /api/health          liveness, database readiness and integration state
 *   POST /api/global/{path}   allow-listed relay to the portal auth API
 *   GET  /api/session         who the caller is, per the portal
 *   *    /api/v1/...          the Appointments API proper — see src/Routes.php
 *   *    /api/public/...      public booking pages, no session — see src/Routes.php
 */
...
if ($path === '' || $path === 'health') {
    ...
    // and integrations blocks are what tell you whether the app can actually be
    ...
    send_json(200, [
        ...
        // One field to read when something is wrong. Calendar is in it because
        // an appointment product that cannot reach a calendar cannot compute
        // availability or write a booking — it is up, and it is not usable.
        'usable' => $database['reachable']
            && ($database['schema']['ready'] ?? false)
            && $integrations['calendar']['configured'],
    ]);
}
...
try {
    ...
    error_log('[appointments] database error on ' . $path . ': ' . $e->getMessage());
    Http::error(503, 'database_unavailable', 'The Appointments database is not reachable right now. Please retry.');
} catch (ResponseSent $e) {
    // Only reachable under CLI, where Http throws instead of exiting.
    send_json($e->status, $e->payload);
} catch (\Throwable $e) {
    error_log('[appointments] unhandled error on ' . $path . ': ' . $e->getMessage() . ' @ ' . $e->getFile() . ':' . $e->getLine());
