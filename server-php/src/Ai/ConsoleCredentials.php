 ...
 * answer for a few minutes. Appointments owns its PROMPTS and its AI LOGIC —
 * see InsightEngine — and Console owns the keys and the provider governance.
 * Those are different things and both statements are true at once.
 *
 * Adapted from the same class in calendar-react-app, deliberately, so the fleet
 * has one way of doing this.
 */
final class ConsoleCredentials
{
    /** The module name this product resolves under, in Console. */
    public const MODULE = 'appointments';
    ...
    public static function status(): array
    {
        ...
        if ($resolved === null) {
            return [
                ...
                'model'      => null,
                'provider'   => null,
                'reason'     => 'AI insights are unavailable. Console did not return credentials for Appointments.',
                'admin_hint' => 'Add an AI connected account for the "appointments" module in Console.',
            ];
        }
    ...
    private static function fetch(string $module): ?array
    {
        ...
        $url = $base . '/ai/credentials/resolve?domain=' . rawurlencode('appointments')
        ...
        if (!is_string($raw) || $status !== 200) {
            ...
            error_log(sprintf(
                '[appointments-ai] Console resolve for %s failed: %s',
                $module,
    ...
    public static function overrideForTesting(?array $credentials): void
    {
        ...
        self::$memo['appointments|' . self::MODULE] = [
            'value'   => $credentials,
            'expires' => time() + 300,
        ];
    }
