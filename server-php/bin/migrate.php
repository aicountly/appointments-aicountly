...
/**
 * Apply pending SQL migrations for the Appointments database.
 *
...
$pdo->exec(
    'CREATE TABLE IF NOT EXISTS appointment_sql_migrations (
...
$applied = [];
foreach (Db::all('SELECT filename, checksum FROM appointment_sql_migrations') as $row) {
...
foreach ($files as $path) {
    ...
    try {
        ...
        $stmt = $pdo->prepare('INSERT INTO appointment_sql_migrations (filename, checksum) VALUES (:f, :c)');
