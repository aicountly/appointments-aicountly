# AICOUNTLY auth workflow (Appointments)

How Appointments signs a user in. This is the shared AICOUNTLY SaaS flow — the same
...
1. User opens `appointments.aicountly.com` (or `appointments.gh.aicountly.com`).
2. No `auth_token` → redirect to
   `{portal}/login/authentication_jump/appointments?returnUrl={origin}/auth/callback`.
...
| Appointments host | Login redirect | Auth API | Product API |
|---|---|---|---|
| `appointments.aicountly.com` | `my.aicountly.com` | `my.aicountly.com` | `appointments.aicountly.com/api` |
| `appointments.gh.aicountly.com` | `sandbox.aicountly.com` | `my.aicountly.com` | `appointments.gh.aicountly.com/api` |
...
`appointments` out of either hostname, and `isSandboxHost()` picks the portal.
...
curl https://appointments.gh.aicountly.com/api/health
...
curl -i -X POST https://appointments.gh.aicountly.com/api/global/seskey

# 3. Unrelayed paths are refused
curl -i -X POST https://appointments.gh.aicountly.com/api/global/login   # expect 404
```
