...
| --- | --- | --- | --- |
| Deploy to cPanel Production | `web/dist/` then `server-php/` | `<remote root>/` and `<remote root>/api/` | https://appointments.aicountly.com (+ `/api`) |
| Deploy to cPanel Sandbox | `web/dist/` then `server-php/` | `<remote root>/` and `<remote root>/api/` | https://appointments.gh.aicountly.com (+ `/api`) |
...
`https://appointments.aicountly.com/api/.env` unless Apache is told otherwise.
